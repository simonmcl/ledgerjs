...
export const receiveAPDU = (rawStream: Observable<Buffer>): Observable<Buffer> => Observable.create((o) => {
  ...
  const sub = rawStream.subscribe({
    complete: () => {
      o.error(new Error("disconnected"));
      sub.unsubscribe();
    },
    ...
    next: (value) => {
      ...
      if (tag !== TagId) {
        o.error(
          ...
          new Error("Invalid tag " + tag.toString(16))
        );
        return;
      }
      ...
      if (notifiedIndex !== index) {
        o.error(
          ...
          new Error("BLE: Invalid sequence number. discontinued chunk. Received " + index + " but expected " + notifiedIndex)
        );
        return;
      }
      ...
      if (notifiedData.length > notifiedDataLength) {
        o.error(
          ...
          new Error("BLE: received too much data. discontinued chunk. Received " + notifiedData.length + " but expected " + notifiedDataLength)
        );
        return;
      }
